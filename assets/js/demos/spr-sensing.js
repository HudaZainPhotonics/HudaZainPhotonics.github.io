...
update();
/* =========================================================
   SPR DEMO — AL-FOLIO / PLOTLY RESPONSIVE RESIZE PATCH
   ========================================================= */

(function () {
  const demo = document.querySelector(".spr-demo");

  if (!demo || typeof Plotly === "undefined") {
    return;
  }

  function resizeAllSprPlots() {
    const plots = demo.querySelectorAll(".js-plotly-plot");

    plots.forEach(function (plot) {
      try {
        Plotly.Plots.resize(plot);
      } catch (error) {
        console.warn("SPR Plotly resize skipped:", error);
      }
    });
  }

  /*
   * Plotly can calculate its width before the al-folio/Jekyll
   * page container has reached its final width.
   *
   * Two animation frames allow the browser to finish layout
   * before Plotly is explicitly resized.
   */
  ...
  /*
   * Run again once all page resources have loaded.
   */
  ...
  /*
   * Keep plots responsive when the browser width changes.
   */
  ...
  /*
   * More importantly, observe the SPR demo container itself.
   * This catches width changes caused by al-folio/Bootstrap
   * even when the browser window has not changed size.
   */
